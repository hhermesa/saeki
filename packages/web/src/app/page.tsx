...
export default function UploadPage() {
  ...
  return (
      <div className="min-h-screen bg-[#FFECD1] flex flex-col items-center pb-8">
        <Stepper
            steps={['Upload','Configure','Checkout']}
            current={step}
            finished={orderSuccess !== null}
            colors={{ completed: 'bg-green-500', active: 'bg-blue-600', upcoming: 'bg-gray-300' }}
        />
        <div className="w-full flex flex-col max-w-xl px-6 grow justify-center items-center pt-6">
          <h1 className="text-3xl font-bold mb-6 text-center w-full">{titles[step]}</h1>

          {step === 0 && (
              <div className="bg-white rounded-lg shadow-lg p-6 mb-6 w-full">
                <form className="space-y-4">
                  <FileUploadSection inputId={FILE_INPUT_ID} files={files} onChange={setFiles} />
                  <button
                      type="button"
                    ...
                    <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
                      <div className="bg-white rounded-lg shadow-lg w-11/12 max-w-md p-6">
                        ...
                        <div className="mt-6 flex justify-end gap-2">
                          ...
                          <button onClick={handleConfirm} className="px-4 py-2 bg-green-600 text-white rounded hover:bg-green-700">Confirm & Place</button>
                        </div>
                      </div>
                    </div>
                )}
